...
| `/update-from-session` | セッション中の失敗・試行錯誤からの学習 | セッション終了前に手動で実行 |
...
### /update-from-session（セッションの学び → ドキュメント更新）

```
ユーザーが /update-from-session を実行
    │
    ▼
...
`/update-from-session` の呼び忘れを防ぐために:

```markdown
- セッション中に失敗や修正指示があった場合、セッション終了前に /update-from-session の実行を提案する
```
...
- **/update-from-session は手動**: セッション履歴が必要なため自動化はできません。
