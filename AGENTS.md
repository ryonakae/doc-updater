...
Stopフック判定にLLMプロンプトを使い、会話的なやり取りや調査報告では発火しない。セッション履歴を参照して以下の2条件を判定する（両方満たす場合のみ続行指示）:
1. 実装完了：コード変更を伴うタスクが完了し、成果が報告されている
2. コード変更あり：ファイルの作成・編集・削除が行われている

git操作のみ・相談調査・途中経過・判断がつかない場合はすべてok=true（発動しない）。タイムアウトは60秒で設定。
...
- **Stopフックのタイムアウト**: 60秒に設定。セッション履歴が大きい場合は調整が必要
