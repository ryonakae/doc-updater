...
Stopフック判定にLLMエージェントを使い、会話的なやり取りや調査報告では発火しない。セッション履歴を参照して以下の2条件を判定する（両方満たす場合のみ続行指示）:
...
- **Stopフックのコスト**: agent型ゲートは毎回LLM呼び出しが発生する。コストが気になる場合はgit pre-commitフックのみの運用も可能
- **Stopフックのタイムアウト**: 180秒（3分）に設定。セッション履歴が大きい場合は調整が必要
