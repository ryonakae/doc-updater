...
└── src/                                  # 配布用ファイル群（他プロジェクトに導入する）
    ...
    │   └── skills/
    │       └── commit-push/
    │           ├── SKILL.md              # doc-updater サブエージェント連携込みのコミット&pushスキル
    ├── .codex/
    │   └── agents/
    │       └── doc-updater.toml          # Codex 用 doc-updater サブエージェント定義と更新手順
    ├── .claude/
    │   ├── agents/
...
- Codex 向け Skills（`src/.agents/skills`）
- Codex（Custom agents / Skills）
- Claude Code（Hooks / Sub-agents / Skills）
- シェルスクリプト（`src/git-hooks/pre-commit`）
- LLMモデル: claude-sonnet-4-6（Stopフック判定用）、haiku（Claude の doc-updater 用）、gpt-5.4-mini（Codex の doc-updater 用）
...
**無限ループ防止**: `stop_hook_active` は同一 Stop イベント内のリエントランス防止フラグであり、次のターンの Stop イベントではリセットされる。そのため、doc-updater 実行後の2回目の Stop イベントでも `stop_hook_active: false` になり再発火しうる。これを防ぐため、`last_assistant_message` が以下のいずれかに該当する場合を例外として ok=true を返す:
- doc-updater サブエージェントの実行結果や完了報告を含む
- ドキュメント更新の結果報告を含む（更新した・更新不要だった等）
- AGENTS.md / CLAUDE.md / README.md 等のドキュメントファイルを編集した報告を含む
- /commit-push スキルの実行結果を含む（スキル内部で doc-updater が実行済みのため）
...
Stopフック判定には claude-sonnet-4-6 を使い、セッション履歴の分析精度を確保する。Claude の doc-updater サブエージェントは haiku、Codex の doc-updater サブエージェントは `gpt-5.4-mini` と `medium` reasoning effort を使う。
...
| ファイル | 役割 |
|---------|------|
| `src/.agents/skills/commit-push/SKILL.md` | Codex 向け。Conventional Commitsメッセージ自動生成 + doc-updaterサブエージェント連携 + push |
| `src/.codex/agents/doc-updater.toml` | Codex 向け。project-scoped な doc-updater サブエージェント定義。更新手順も内包する |
...
cp -r src/.codex/ /path/to/your-project/.codex/
cp -r src/.claude/ /path/to/your-project/.claude/
# 既存の .claude/settings.json がある場合は hooks セクションだけマージ
```

Codex では `.agents/skills/commit-push` と `.codex/agents/doc-updater.toml` をセットで配置する。`doc-updater.toml` にドキュメント更新の手順を直接書く。
...
`src/.claude/agents/doc-updater.md` と `src/.codex/agents/doc-updater.toml` の記述規約セクションはテンプレート。プロジェクトのドキュメント構造に合わせてカスタマイズする:
